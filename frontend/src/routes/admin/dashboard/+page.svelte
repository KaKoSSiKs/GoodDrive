<script>
  ...
  async function loadStats() {
    try {
      ...
      stats.totalOrders = ordersStats.total_orders || 0;
      
      // Подсчёт заказов по статусам
      const statusStats = ordersStats.status_statistics || [];
      stats.newOrders = statusStats.find(s => s.status === 'new')?.count || 0;
      stats.processingOrders = statusStats.find(s => s.status === 'processing')?.count || 0;
      stats.shippedOrders = statusStats.find(s => s.status === 'shipped')?.count || 0;
      stats.completedOrders = statusStats.find(s => s.status === 'completed')?.count || 0;
      stats.canceledOrders = statusStats.find(s => s.status === 'canceled')?.count || 0;
